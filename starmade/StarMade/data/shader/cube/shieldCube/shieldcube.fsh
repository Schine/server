...
void main(void) {
	...
	alphaPerc = hit(dist0, alphaPerc, 0);
	...
	vec4 noise = texture2D(m_Noise, (gl_TexCoord[0].st + m_Time)/2.0); 	//load du/dv normalmap //+noise.xy/2.0
	//removed distortion offset breaks tiling
	//vec4 distort = texture2D(m_Distortion, noise.xy * gl_TexCoord[0].st/100.0); 	//load du/dv normalmap //+noise.xy/2.0
